  ...
  export const useChatStore = create((set, get) => {
    ...
    return {
      ...
      /* ---------- crypto init (RESTORES initCrypto) ---------- */
      initCrypto: async () => {
        ...
        try {
          ...
          console.warn("Failed to import private key, regenerating", err);
        }
        ...
        if (!keyPair) {
          ...
          try {
            ...
            const id = useStore.getState().authUser?._id || "unknown";
            ...
            console.warn("Export private key failed:", e);
          }
        }
  ...
  getMessages: async (userId) => {
    try {
      ...
      // ✅ Auto-mark as read once messages are fetched
      ...
      /* ---------- selection (RESTORES setSelectedUser) ---------- */
      setSelectedUser: async (user) => {
        ...
        if (socket) {
          const me = useStore.getState().authUser;
          if (me) socket.emit("mark_as_read", { senderId: user._id, receiverId: me._id });
        }
      },
...
socket.on("newMessage", async ({ from, msg }) => {
  ...
  if (selected && String(selected._id) === chatUserId) {
    set({ messages: get().messagesByUser[chatUserId] });
  }
